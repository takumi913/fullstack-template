...
export default function ProfileSettingsPage() {
  ...
  return (
    <SettingsPage title="个人资料" description="这些信息会显示在你加入的工作区中。">
      <form onSubmit={submit} className="panel max-w-2xl p-6">
        <div className="space-y-5">
          ...
          <Input label="头像 URL" value={avatar} onChange={setAvatar} required={false} />
        ...
        <div className="mt-6 flex items-center gap-3 border-t pt-5">
          ...
          {saved && <span className="text-sm text-zinc-500">已保存</span>}
        </div>
      </form>
    </SettingsPage>
  );
    ...
    <div className="shell py-12 sm:py-16">
      <div className="mb-9 flex flex-col gap-5 border-b pb-7 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex gap-4 text-sm">
          <Link className="text-zinc-500 hover:text-zinc-950" to="/dashboard">
            Dashboard
          </Link>
          <Link className="text-zinc-500 hover:text-zinc-950" to="/settings/security">
            安全
          </Link>
        </div>
      </div>
